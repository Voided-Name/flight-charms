    <div class="conatiner-fluid content-inner mt-n5 py-0">
      <div class="row">
        <div class="col-md-12 col-lg-12">
          <div class="row">
            <div class="col-md-12">
              <div class="card" data-aos="fade-up" data-aos-delay="800">
                <div class="flex-wrap row d-flex justify-content-evenly align-items-center p-3">
                  <a href="<?= Flight::request()->base ?>/dashboard/employer/jobVacancies" class="col-md-12 col-lg-5 btn btn-primary p-2 m-2 fs-1">
                    View Job Vacancies Created
                  </a>
                  <a href="<?= Flight::request()->base ?>/dashboard/employer/createVacancy" class="col-md-12 col-lg-5 btn btn-secondary p-2 m-2 fs-1">
                    Create New Job Vacancy
                  </a>
                  <a href="<?= Flight::request()->base ?>/dashboard/employer/allVacancies" class="col-md-12 col-lg-5 btn btn-success p-2 m-2 fs-1">
                    Job Vacancies
                  </a>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
